...
def IOU(boxes, anchor):
    """
        計算anchor和boxes之間的IOU
    :param boxes: [N, 2]
    :param anchor: [K, 2]
    :return:
    """

    w_min = np.minimum(boxes[:, 0, None], anchor[None, :, 0])  # [N, K]
    h_min = np.minimum(boxes[:, 1, None], anchor[None, :, 1])  # [N, K]
    inter = w_min * h_min  # [N, K]

    boxes_area = boxes[:, 0] * boxes[:, 1]  # [N]
    anchor_area = anchor[:, 0] * anchor[:, 1]  # [K]

    union = boxes_area[:, None] + anchor_area[None, :]  # [N, K]
